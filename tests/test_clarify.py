...
from starter.clarify import (ALLOWED_ATTRIBUTES, ASK_VALUE_FLOOR,
                             SCORABLE_ATTRIBUTES, WILDCARD,
                             ClarificationLedger, attribute_value, choose,
                             rank_attributes)
...
class AskDecisionTest(unittest.TestCase):
    ...
    def test_a_pool_with_no_signal_falls_back_to_the_open_question(self) -> None:
        candidates, metadata = _pool({}, {})
        self.assertEqual(
            choose(_context(), ClarificationLedger(), candidates, metadata,
                   None),
    ...
    def test_missing_metadata_is_the_open_question_not_a_crash(self) -> None:
        candidates = [Candidate(parent_asin="A")]
        self.assertEqual(
            choose(_context(), ClarificationLedger(), candidates, None, None),
            WILDCARD)

    def test_a_value_below_the_floor_falls_back(self) -> None:
...
class NoRepeatedQuestionsTest(unittest.TestCase):
    ...
    def test_a_productive_question_may_be_asked_again(self) -> None:
        # "And what else?" is not a repeated question. Only an attribute that
        # answered EMPTY is closed -- see CP 15.7 below.
        candidates, metadata = _pool({}, {})
        ledger = ClarificationLedger()
